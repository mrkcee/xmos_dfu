...
If your Element IV is running firmware older than v1.5.1, flash **v1.5.1 first**, then flash **v1.7.4**.
...
5. Confirm with `y` when prompted. The process take about a minute.
6. You will be prompted at the end to flash again or exit: Press `n` to quit, and then Exit via `5`. 
...
However, around 1% of devices have been found to fail to upgrade beyond `v1.5.1`. This is believed to be a WinUSB driver limitation: some flash chips time out during their erase cycle via WebUSB for firwmare > `v1.5.1`. If your firmware update fails or stalls at 0% through core.jdslabs.com, there are three possible solutions. 
